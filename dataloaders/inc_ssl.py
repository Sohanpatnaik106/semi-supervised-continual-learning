...
class iCIFAR10(data.Dataset):
    ...
    def __init__(self, root, dataset, num_labeled=None, num_unlabeled_pt=0,
        ...
        self.seed = seed
        self.t = -1
        self.l_dist = l_dist    # super
        self.ul_dist = ul_dist  # random
        ...
        if self.l_dist == 'super':
            ...
            for super_k in np.unique(self.course_targets):
                ...
                ind_task = np.where(self.course_targets == super_k)[0]  # Obtain all image indices corresponding to a parent class
                ind_task_labels = [self.targets[ind_task[i]] for i in range(len(ind_task))]     # Obtain the corresponding class labels
