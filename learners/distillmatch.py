...
class DistillMatch(NormalNN):
    ...
    def __init__(self, learner_config):
        ...
        # fm parameters
        self.dynamic_threshold = learner_config['dynamic_threshold']
    ...
    def get_dynamic_threshold(self, prob_xu, wu, yu, c):
        
        # NOTE: Add warm-up, add non linear mapping
        # TODO: 
        #   1. Without warm up
        #   2. With warm up
        ...
        for c in classes:
            a = np.array((np.max(np.array(prob_xu.cpu()), axis = 1) > self.fm['thresh']))
    ...
    # fixmatch / flexmatch loss for semi-supervised learning
        ...
        if len(self.dc_eps_values) == 1:
            ...
            for m in magnitude_list:
                ...
                for i, (xl, y, xul, yul, task)  in enumerate(train_loader):
                    ...
                    # print(inputs.grad.data.shape)
                    # 
                    gradient = torch.ge(inputs.grad.data, 0)
                    gradient = (gradient.float() - 0.5) * 2
                    # DOUBT: What is this modified input
